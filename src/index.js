...
var EngineWrapper = require("./engine-wrapper") // fly
function adapter() {
  return function(config, cb) {
    var mockAdapter = this;
    return new Promise(function(resolve, reject) {
      var response = handleRequest(mockAdapter, resolve, reject, config)
...
function MockAdapter(axiosInstance, options) {
  ...
  if (axiosInstance) {
    ...
    this.originalAdapter = axiosInstance.engine;
    this.delayResponse = options && options.delayResponse > 0 ? options.delayResponse : null;
    axiosInstance.engine = EngineWrapper(this.adapter.call(this));
  }
}
...
MockAdapter.prototype.restore = function restore() {
  if (this.axiosInstance) {
    this.axiosInstance.default.engine = this.originalAdapter;
  }
};
